...
get_latest_ckpt() {
    ...
    # Filter for directories only to avoid matching log files, and require strict naming suffix
...
submit_job() {
    ...
    # Hard cap at 400 epochs; checkpoint evaluation happens every epoch
    ...
    # Determine which dataset to use based on the config name
    if [[ "$CONFIG" == *"did.yaml" ]]; then
        DATA_DIR="data/augmented_did_h5"
    else
        DATA_DIR="data/augmented"
    fi
    
    # Submit train_ldm.slurm which is now in scripts/ relative to root
    sbatch --export=ALL,EXTRA_ARGS="$FINAL_ARGS",DATA_DIR="$DATA_DIR" scripts/train_ldm.slurm
}
...
submit_job "pinns_baseline_base" "-n pinns_baseline_base \
 ...
 submit_job "physics_hybrid_master_did" "-n physics_hybrid_master_did \
 model.params.use_pinn_loss=True \
 model.params.pinn_loss_weight=0.5 \
 model.params.grad_corr_weight=0.5 \
 model.base_learning_rate=5.0e-7 \
 ...
 lightning.callbacks.image_logger.params.batch_frequency=10000 \
 lightning.modelcheckpoint.params.save_top_k=1 \
 lightning.trainer.log_every_n_steps=50" "configs/latent-diffusion/cfd_ldm_pinnsformer_did.yaml"
...
COMMENT
 ...
 submit_job "pinns_baseline_wavelet_did" "-n pinns_baseline_wavelet_did \
 model.params.use_pinn_loss=True \
 model.params.pinn_loss_weight=0.1 \
 model.params.lambda_res=1.0 \
 model.params.lambda_bc=1.0 \
