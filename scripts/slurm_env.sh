...
activate_ldm_env() {
  ...
  if [[ ! -d "$env_path" ]]; then
    ...
    echo "  sbatch scripts/create_ldm_env_pace.slurm" >&2
    echo "Expected env location: $env_path" >&2
    echo "Or set LDM_ENV_NAME / LDM_ENV_PATH to an existing environment." >&2
    exit 2
  fi
...
import sys
missing = []
for name in ("torch", "pytorch_lightning", "omegaconf", "h5py", "numpy"):
    try:
        __import__(name)
